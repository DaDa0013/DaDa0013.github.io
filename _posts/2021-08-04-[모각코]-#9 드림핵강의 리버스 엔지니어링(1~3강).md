...
![image-20210814225907513](image/image-20210814225907513.png)
...
* **Disassemble(디스어셈블)**: 바이너리 코드를 어셈블리 코드로 변환하는 과정
  ...
  ![image-20210814230404543](image/image-20210814230404543.png)
...
* **x64의 범용 레지스터**
  ...
  ​	![image-20210814231117471](image/image-20210814231117471.png)
...
![image-20210814232851321](image/image-20210814232851321.png)
...
* **FLAGS**: 현재 상태나 조건을 0과 1로 나타내는 상태 레지스터
  ...
  | ----------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
  | CF(Carry Flag)    | **자리 올림(carry)이 생기는 경우 CF의 값이 1**이 됨<br />CF의 특징: **연산에 사용된 값들에 부호가 없음(unsigned)** |                                                              |
  | ZF(Zero Flag)     | **연산의 결과가 0일 때 ZF는 1이 됨.**                        | ![img](https://kr.object.ncloudstorage.com/dreamhack-content/page/c6041f8ff5f5953cf0118ca70826739439736fad4abe7b5de307d180b247b784.png) |
  | SF(Sign Flag)     | 수행한 결과가 양수일 때, 즉 **최상위 비트가 0이면 SF=0**, 반대로 결과가 음수가 되어 **최상위 비트가 1이면 SF=1**이 됨 | ![img](https://kr.object.ncloudstorage.com/dreamhack-content/page/2823463df8237c1d8fdbcbd05744da72250a1181acf85cb1e7d830c79c96a83b.png) |
  | OF(Overflow Flag) | ‘overflow’ 라는 이름에서 알 수 있듯이 부호 있는 값들을 대상으로 산술 연산을 했을 때 자리 올림이 생겼다는 것은 **표시할 수 있는 값의 범위를 넘어갔다(overflowed)는 것**을 의미 | ![img](https://kr.object.ncloudstorage.com/dreamhack-content/page/2f2a541a46014fa31aa505b0f6a1b8191c26efe311a823947c8dd2ba8723f5e5.png) |
